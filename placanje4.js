...
},
{
  ...
  let generateCartItems = () => {
    if (basket.length !== 0) {
        ...
        // Retrieve the shipping option from local storage
        const shippingOption = localStorage.getItem('shippingOption') || 'Not selected';
        // Splitting the shipping option to show the method separately under "Ime Proizvoda"
        const shippingOptionDisplay = shippingOption.split('+')[0]; // This extracts the shipping method name
        const shippingCost = shippingOption.split('+')[1] || '0 KM'; // This extracts the shipping cost, defaults to '0 KM' if not found
...
let shopItemsData = JSON.parse(localStorage.getItem('shopItemsData')) || [];
  ...
  if (basket.length !== 0) {
      let baseAmount = basket
          ...
          let selectedOption = localStorage.getItem('shippingOption');
      let surcharge = selectedOption === "LegerMax +10 KM" ? 10 : 0;
      let totalAmount = baseAmount + surcharge;
        ...
        document.getElementById("LegerMax +10 KM").addEventListener("change", updateTotalAmount);
        document.getElementById("Preuzece u jet").addEventListener("change", updateTotalAmount);
